...
export const Navbar = ({ activeView, setView, toggleTheme, theme }) => {
    const navItems = [
        { name: 'Home', icon: Home, view: 'home' },
        { name: 'IPO TraQ', icon: Rocket, view: 'explore' },
        { name: 'Insight TraQ', icon: MessageSquare, view: 'chatbot' },
        { name: 'Roadmap', icon: Milestone, view: 'roadmap' },
    ];
                    ...
                    <div className="hidden md:flex items-center space-x-4">
                        {navItems.map((item) => (
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-200 ${activeView === item.view
